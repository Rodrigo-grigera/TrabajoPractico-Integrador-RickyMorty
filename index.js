...
const getPersonajes = async () =>{
    try{
        const respo = await fetch (`${API}?page=${paginaActual}&name=${nombre}&status=${estado}`);
        if(respo.ok){
            const personaje = await respo.json();
            // console.log(personaje.results);
            // console.log (personaje.info.pages);
